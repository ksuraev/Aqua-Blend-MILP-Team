...
\begin{equation*}
    ...
    ~ \underbrace{\le H_{t r_i p} \sum_{s \in \mathcal S} Q_{sp} b_{st}}_{\eqref{eq:removal_process_initial}}
    ~ \underbrace{\le H_{t r_i p}\,\overline{I}_{tp} \sum_{s \in \mathcal{S}} b_{st}}_{\eqref{eq:inlet_treatability}}
    ~ \underbrace{\le H_{t r_i p}\,\overline{I}_{tp}\,\overline{V}_t \beta_t}_{\eqref{eq:plant_capacity}},
\end{equation*}
so \eqref{eq:removal_activation} restricts the model only through $\sigma_{t r_i} = 0$.
...
\textcolor{red}{water in equals water out, and removal doesn't change it by the assumption of no water loss so $\sum_s a_s$ still leaves plant and I think should remain that label because that's the volume of water leaving the plant. So maybe just one row in table.The single plant here will have a process sequence $r_{1}, r_{2}, r_{3},..$, i.e. $\mathcal{R}_t$ fixed and one process sequence. $\sigma_{tr}$ and $e_{trp}$ remain free}
