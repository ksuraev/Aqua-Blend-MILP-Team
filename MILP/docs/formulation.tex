...
\end{tabularx}
$\sigma_{tr}$ and $e_{trp}$ are defined only for $r \in \mathcal{R}_{tp}$, and
$\ell^i_{tp}$ only for $i = 0, \dots, n_{tp}$; the objective and constraints below reflect this.
Since we have $\mathcal{R}_{tp} = (r_1, \dots, r_{n_{tp}})$ (Table~\ref{tab:sets}), the process at stage $i$ is $r_i$:
...
\begin{equation}\label{eq:objective}
    \begin{aligned}
        \min \quad
                     & \sum_{s \in \mathcal{S}}
        F_s \alpha_s &                          & \text{source fixed cost} \\ + \; & \sum_{s \in \mathcal{S}} C_s a_s & & \text{source draw cost} \\ + \; & \sum_{t \in \mathcal{T}} F_t \beta_t & & \text{plant fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} C_t \sum_{s \in \mathcal{S}} b_{st} & & \text{plant throughput cost} \\ + \; & \sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st}\, b_{st} + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz}\, c_{tz} & & \text{arc transfer cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} F_{tr} \sigma_{tr} & & \text{removal process fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{p \in \mathcal{P}} \sum_{r \in \mathcal{R}_{tp}} C_{trp}\, e_{trp} & & \text{removal cost} \\\end{aligned} \end{equation}
...
\begin{equation}\label{eq:nonnegativity}
    ...
    r \in \mathcal{R}_{tp},\, p \in \mathcal{P},\, i = 0, \dots, n_{tp}.
\end{equation}
...
For parameter $p$, $\ell^{n_{tp}}_{tp}$ is the load remaining in the treated water, while $\sum_s b_{st}$ is the corresponding water volume, ensuring the volume-weighted average value of $p$ leaving the plant is between $\underline{Q}_p$ and $\overline{Q}_p$.
...
We confine ourselves to ``removal processes'', though our model can be extended to include more treatment processes, e.g. ``chemical dosing'', and thus, this section will be adapted to be consistent with the existing parameter notation while preserving the linear structure of the MILP.
...
We highlight here that, according to \cite{who2004water}, removal process can only reduce alkalinity, though it can be increased by an adjustment process, which is not currently modelled.
...
As mentioned earlier, pH cannot be handled by removal process; however, constraints from \eqref{eq:removal_process_initial} to \eqref{eq:outlet_water_quality} still hold, by setting $H_{tr,\mathrm{pH}} = 0, \forall r \in \mathcal R_t$.
Hence, the constraint \eqref{eq:inlet_treatability} is also valid. Formally, this is:
...
\end{equation*}
Followed from that, $\mathcal R_{t, \mathrm{pH}} = \emptyset$ and $n_{t, \mathrm{pH}} = 0$, and consequently, $\ell^{n_{tp}}_{t, \mathrm{pH}} = \ell^0_{t, \mathrm{pH}}$.
...
\begin{enumerate}[label=(\roman*), align=left, widest=viii]
    ...
    \item Water quality is checked at each treatment plant, incoming quality is validated against $\overline{I}_{tp}$ and $\underline{I}_{tp}$ \eqref{eq:inlet_treatability} - \eqref{eq:inlet_treatability_neutral},
            whereas outcoming quality is validated against $\overline{Q}_p$ and $\underline{Q}_p$ \eqref{eq:outlet_water_quality}.
\end{enumerate}
...
the arc variables are fixed by conservation, and only the source decisions ($\alpha_s$ and $a_s$) and treatment decisions, e.g. $\sigma_{tr}$ and $e_{trp}$ in removal process, remain free.
