...
\begin{tabularx}{\textwidth}{@{}lX@{}}
	...
	$\mathcal{R}_t$   & all processes installed at plant $t$ regardless of what parameters being removed, $\mathcal R_t \subseteq \mathcal{R}$            \\
	$\mathcal{R}_{tp}$   & processes installed at plant $t$ acting on parameter $p$, as an ordered sequence
...
For simplicity, we shall insist on each removal process $r_i \in \mathcal R_{tp}$ acts on exactly one $p \in \mathcal P$, see \textbf{Sec. \ref{sec:removal_process_axiom}} for further assumptions of removal processes.
Then, from the description of $\mathcal R_t$ and $\mathcal R_{tp}$ (Table \ref{tab:sets}), we must validate that:
\begin{equation}
	\label{eq:process_union_intersect}
	\mathcal R_t = \bigcup_{p \in \mathcal P} \mathcal R_{tp} \qquad \text{and} \qquad
	\bigcap_{p \in \mathcal P} \mathcal R_{tp} = \emptyset
\end{equation}
...
%     Should we also use $c_{st}$ instead of $b_{st}$?  \color{blue}{I think in future yes but at the moment it would put all code/docs out of sync?}
% }
...
\begin{equation}\label{eq:objective}
	\begin{aligned}
		\min \quad
		             & \sum_{s \in \mathcal{S}}
		F_s \alpha_s &                          & \text{source fixed cost} \\ + \; & \sum_{s \in \mathcal{S}} C_s a_s & & \text{source draw cost} \\ + \; & \sum_{t \in \mathcal{T}} F_t \beta_t & & \text{plant fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} C_t \sum_{s \in \mathcal{S}} b_{st} & & \text{plant throughput cost} \\ + \; & \sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st}\, b_{st} + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz}\, c_{tz} & & \text{arc transfer cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} F_{tr} \sigma_{tr} & & \text{removal process fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} \sum_{p \in \mathcal{P}} C_{trp}\, e_{trp} & & \text{removal cost} \\\end{aligned} \end{equation}
...
For a source-to-plant arc, the flow $b_{st}$ is capped at $\overline{L}_{st}$ and is zero unless the source is active ($\alpha_s = 1$).
...
\end{equation}
Similarly, a plant-to-zone arc's flow $c_{tz}$ is capped at $\overline{L}_{tz}$ and is zero unless the plant is active ($\beta_t = 1$).
...
Remarkably, with only removal process installed, infeasibility is an immediate result since removal processes cannot increase $p \in \mathcal P$.
...
the arc variables are fixed by conservation, and only the source decisions ($\alpha_s$ and $a_s$) and treatment decisions, e.g. $\sigma_{trp}$ and $e_{trp}$ in removal process, remain free.
