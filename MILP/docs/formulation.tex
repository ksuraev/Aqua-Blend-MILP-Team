...
\textcolor{red}{water in equals water out, and removal doesn't change it by the assumption of no water loss so $\sum_s a_s$ still leaves plant and I think should remain that label because that's the volume of water leaving the plant.
	So maybe just one row in table.The single plant here will have a process sequence $r_{1}, r_{2}, r_{3},.
		.$, i.e. $\mathcal{R}_t$ fixed and one process sequence. $\sigma_{tr}$ and $e_{trp}$ remain free}

\textcolor{magenta}{I added a row to clarify $\sigma_{tr} = 1$ as there is only 1 plant, but hmmm, why $e_{trp}$ is free?}

\textcolor{blue}{$e_{trp}$ is the load removed by the process, which is a decision variable.
	It is not fixed because it depends on the incoming load and the process efficiency.}

\textcolor{blue}{I don't think $\sigma_{tr} = 1$ belongs in this table: with one plant and one zone, $b_{st}$ and $c_{tz}$ are fixed by conservation, there's nowhere else for the water to go but having one plant doesn't force its processes to run.
	If a process is expensive and the blend already complies without it, the model switches it off.
	so I guess the only thing that changes is the first paragraph here and then remove $\gamma_{st}$ and $\delta_{tz}$ from the table since we have removed that earlier?}

\color{blue}{E.g. (feel free to not use this )
	...
	the arc flows are fixed by conservation, the source and treatment decisions remain free.
	...
	\begin{center}
		\begin{tabular}{@{}ll@{}}
			...
			$\beta_t = 1$                                & demand is positive, so $F_t$ is a constant and drops out \\
			\bottomrule
		\end{tabular}
	\end{center}
	then maybe after table:
	...
	The treatment decisions are not fixed by this reduction:
	$\sigma_{t r_i}$ and $e_{t r_i p}$ remain free, so the model still chooses which processes to run and how much to remove.
	With routing determined, the trade-off the toy case exercises is between the cost of the water drawn and the cost of removing what it carries.
}
...
\begin{center}
	\begin{tabular}{@{}ll@{}}
		...
		$b_{st} = a_s$                               & by source flow conservation                                      \\
		$c_{tz} = \sum_{s} a_s$                      & by plant flow conservation                                       \\
		$\sigma_{tr} = 1$                            & one plant, one fixed $\mathcal R_t$                              \\
		\bottomrule
	\end{tabular}
\end{center}
...
\clearpage
\textcolor{red}{put appendix before references?}
\appendix
\input{appendix.tex}
