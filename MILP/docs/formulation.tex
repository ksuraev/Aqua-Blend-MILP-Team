...
\begin{tabularx}{\textwidth}{@{}lX@{}}
    ...
    flows through them                                                           \\
    $\mathcal{C}$     & set of treatment chemicals and additives, $c \in \mathcal{C}$ \\
\end{tabularx}
...
\begin{tabularx}{\textwidth}{@{}lXl@{}}
    ...
    $A_{tc}$             & $1$ if chemical $c$ is available for dosing at plant $t$; $0$ otherwise                               & ---                      \\
...
Chemical dosing is charged according to the mass of each chemical used.
...
\begin{equation}
    \begin{aligned}
        \min \quad
                     & \sum_{s \in \mathcal{S}}
        F_s \alpha_s &                          & \text{source fixed cost} \\ + \; & \sum_{s \in \mathcal{S}} C_s a_s & & \text{source draw cost} \\ + \; & \sum_{t \in \mathcal{T}} F_t \beta_t & & \text{plant fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} C_t \sum_{s \in \mathcal{S}} b_{st} & & \text{plant throughput cost} \\ + \; & \sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st}\, b_{st} + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz}\, c_{tz} & & \text{arc transfer cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} F_{tr} \sigma_{tr} & & \text{removal process fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} \sum_{p \in \mathcal{P}} C_{trp}\, e_{trp} & & \text{removal cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{c \in \mathcal{C}} C^{\mathrm{chem}}_{tc}\, m_{tc} & & \text{chemical dosing cost}
    \end{aligned}
\end{equation}
...
\begin{equation}\label{eq:nonnegativity}
    a_s,\; b_{st},\; c_{tz},\; e_{trp},\; \ell^i_{tp},\; m_{tc} \;\ge\; 0,
    \qquad \forall s \in \mathcal{S},\, t \in \mathcal{T},\, z \in \mathcal{Z},\,
    r \in \mathcal{R}_t,\, p \in \mathcal{P},\, c \in \mathcal{C},\, i = 0, \dots, n_t.
\end{equation}
...
\paragraph{Chemical dosing activation and availability.}
Chemical $c$ can be dosed at plant $t$ only when the plant is active and the chemical is available at that plant.
...
\begin{equation}\label{eq:chemical_dose_lower}
    ...
    \forall t \in \mathcal{T},\ \forall c \in \mathcal{C}.
\end{equation}
When $\kappa_{tc}=1$, constraints~\eqref{eq:chemical_dose_upper} and~\eqref{eq:chemical_dose_lower} reduce to the required dose range.
When $\kappa_{tc}=0$, constraint~\eqref{eq:chemical_mass_activation} forces $m_{tc}=0$, while the lower bound is relaxed.
...
\begin{equation}\label{eq:post_treatment_load}
    ...
    \ell^{n_t}_{tp}
    +
    \sum_{c \in \mathcal{C}} \eta_{cp} m_{tc},
    \qquad
...
The initial chemical dosing formulation does not assume that adding a coagulant produces a fixed linear reduction in turbidity; such a treatment response requires a separately justified or calibrated relationship and is therefore not represented by $\eta_{c,\mathrm{turb}}$ unless supporting data are available.
...
If the acceptable raw pH range is
\begin{equation}\label{eq:raw_ph_range}
    ...
    \overline{\mathrm{pH}},
\end{equation}
the corresponding transformed regulatory bounds are
\begin{equation}\label{eq:ph_bound_transformation}
    \underline{Q}_{\mathrm{pH}}
    =
    10^{-\overline{\mathrm{pH}}},
    \qquad
    \overline{Q}_{\mathrm{pH}}
    =
    10^{-\underline{\mathrm{pH}}}.
\end{equation}
...
The optimisation model constrains pH through the transformed source value $Q_{s,\mathrm{pH}}$.
...
Because $\eta_{cp}$ is a fixed parameter and $m_{tc}$ is a continuous decision variable, the relationship in Equation~(33) remains linear and is therefore compatible with the MILP formulation.
...
\begin{enumerate}[label=(\roman*), align=left, widest=xii]
    ...
    \item Raw pH values are not averaged directly; pH is transformed to hydrogen ion concentration before optimisation.
...
\begin{itemize}
    ...
    \item Chemical dosing is optional and can occur only when the chemical is available and the plant is active.
