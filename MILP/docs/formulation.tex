...
\begin{tabularx}{\textwidth}{@{}lX@{}}
    ...
    $\mathcal{R}_t$   & set of all removal processes installed at plant $t$, with $\mathcal{R}_t \subseteq \mathcal{R}$ \\
    $\mathcal{R}_{tp}$ & ordered sequence of removal processes installed at plant $t$ that act on parameter $p$,
    written as $r_1,\dots,r_{n_{tp}}$ in the order water flows through them        \\
...
\end{tabularx}
$n_{tp}=|\mathcal{R}_{tp}|$ is indexed by both plant and quality parameter because different processes at the same plant may act on different parameters.
For example, a plant may have several installed processes in $\mathcal{R}_t$, while only filtration appears in $\mathcal{R}_{t,\mathrm{turb}}$ and a different subset appears in $\mathcal{R}_{t,\mathrm{alk}}$.
...
\end{tabularx}
The process activation variable $\sigma_{tr}$ is defined for each installed process $r\in\mathcal{R}_t$.
The removal variable $e_{trp}$ is defined only for $r\in\mathcal{R}_{tp}$, and
$\ell^i_{tp}$ only for $i=0,\dots,n_{tp}$.
Since $\mathcal{R}_{tp}=(r_1,\dots,r_{n_{tp}})$ is the ordered sequence relevant to parameter $p$, the process at stage $i$ is $r_i$ and $e_{t r_i p}$ denotes the corresponding removal variable.
A process in $\mathcal{R}_t$ may appear in more than one $\mathcal{R}_{tp}$ if it acts on more than one quality parameter, while its activation decision $\sigma_{tr}$ remains shared across those parameters.
...
\begin{center}
    ...
    \captionof{figure}{One example of the network structure (sources $\to$ treatment plants $\to$ demand zones).
        Each source-plant arc carries a flow variable $b_{st}$ and each plant-zone arc carries a flow variable $c_{tz}$.
...
\begin{equation}
    \begin{aligned}
        \min \quad
                     & \sum_{s \in \mathcal{S}}
        F_s \alpha_s &                          & \text{source fixed cost} \\ + \; & \sum_{s \in \mathcal{S}} C_s a_s & & \text{source draw cost} \\ + \; & \sum_{t \in \mathcal{T}} F_t \beta_t & & \text{plant fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} C_t \sum_{s \in \mathcal{S}} b_{st} & & \text{plant throughput cost} \\ + \; & \sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st}\, b_{st} + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz}\, c_{tz} & & \text{arc transfer cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} F_{tr} \sigma_{tr} & & \text{removal process fixed cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{p \in \mathcal{P}} \sum_{r \in \mathcal{R}_{tp}} C_{trp}\, e_{trp} & & \text{removal cost} \\ + \; & \sum_{t \in \mathcal{T}} \sum_{c \in \mathcal{K}_t} C^{\mathrm{chem}}_{tc}\, m_{tc} & & \text{chemical dosing cost}
    \end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    \ell^i_{tp} &\ge 0,
    && \forall t \in \mathcal{T},\ \forall p \in \mathcal{P},\ i = 0,\dots,n_{tp},\\
    m_{tc} &\ge 0,
...
Because the current formulation does not assign fixed activation costs to transfer links, separate arc-activation binaries are not required.
A source-to-plant flow is bounded by the arc capacity, the source capacity, and the plant capacity, and is forced to zero when the source is inactive:
\begin{equation}\label{eq:source_plant_arc_capacity}
    b_{st}
    \le
    \min\!\left\{
        \overline{L}_{st},
    ...
    \forall s\in\mathcal{S},\ \forall t\in\mathcal{T}.
\end{equation}
If plant $t$ is inactive, constraint~\eqref{eq:plant_capacity} forces all incoming $b_{st}$ to zero, so a separate plant-activation binary on the source-to-plant arc is unnecessary.

Similarly, a plant-to-zone flow is bounded by the arc capacity and the plant capacity and is forced to zero when the plant is inactive:
\begin{equation}\label{eq:plant_zone_arc_capacity}
    c_{tz}
    \le
    \min\!\left\{
        \overline{L}_{tz},
    ...
    \forall t\in\mathcal{T},\ \forall z\in\mathcal{Z}.
\end{equation}
If fixed arc-activation costs or other link-specific on/off conditions are introduced in a future extension, separate arc binary variables can be reintroduced.
...
For each quality parameter $p$, only the installed processes that act on that parameter are included in the ordered sequence $\mathcal{R}_{tp}$.
Processes in $\mathcal{R}_t\setminus\mathcal{R}_{tp}$ leave the load of $p$ unchanged and therefore do not require a separate load-balance stage for that parameter.
Denoting the load of $p$ remaining after the $i$-th relevant process as $\ell^i_{tp}$, the load entering the sequence is
...
\end{equation}
For $\mathcal{R}_{tp}=(r_1,\dots,r_{n_{tp}})$, each relevant process removes part of the load it receives and passes on the remainder:
\begin{equation}\label{eq:removal_process_balance}
    ...
    i=1,\dots,n_{tp}.
\end{equation}
The load remaining after all removal processes relevant to parameter $p$ is $\ell^{n_{tp}}_{tp}$.
When $\mathcal{R}_{tp}=\varnothing$, $n_{tp}=0$ and the final removal-stage load is simply $\ell^0_{tp}$.

\paragraph{Removal capability.}
Each relevant process removes no more than a fixed fraction $H_{t r_i p}$ of the load arriving at that stage.
...
\begin{equation}\label{eq:removal_capability}
    e_{t r_i p}
    \le
    H_{t r_i p}\,\ell^{i-1}_{tp},
    \qquad
    \forall t\in\mathcal{T},\ \forall p\in\mathcal{P},\
    i=1,\dots,n_{tp}.
\end{equation}
A process $r\in\mathcal{R}_t$ that does not act on parameter $p$ is excluded from $\mathcal{R}_{tp}$; equivalently its removal effect on $p$ is zero.
By \eqref{eq:nonnegativity} and \eqref{eq:removal_process_balance}, no sequence can remove more load than is available.

\paragraph{Removal activation.}
No removal occurs unless the corresponding installed process is active.
For each $r_i\in\mathcal{R}_{tp}$,
\begin{equation}\label{eq:removal_activation}
    ...
    i=1,\dots,n_{tp}.
\end{equation}
When $\sigma_{t r_i}=0$, this forces $e_{t r_i p}=0$.
When $\sigma_{t r_i}=1$, the bound is valid and does not further restrict the removal capability because
\[
\begin{aligned}
e_{t r_i p}
&\le H_{t r_i p}\ell^{i-1}_{tp}
\le H_{t r_i p}\ell^0_{tp} \\
&= H_{t r_i p}\sum_{s\in\mathcal{S}}Q_{sp}b_{st}
\le H_{t r_i p}\overline{I}_{tp}\sum_{s\in\mathcal{S}}b_{st} \\
&\le H_{t r_i p}\overline{I}_{tp}\overline{V}_t\beta_t
\le H_{t r_i p}\overline{I}_{tp}\overline{V}_t.
\end{aligned}
\]
...
This is why, where a plant has no genuine inlet limit on $p$, $\overline{I}_{tp}$ is set to the largest value reachable from connected sources rather than to an arbitrary large constant.
...
Chemical dosing is applied after the ordered parameter specific removal process sequence $\mathcal{R}_{tp}$ in the initial formulation.
...
If $\mathcal{R}_{tp}=\varnothing$, then $n_{tp}=0$ and chemical dosing is applied directly to the incoming load $\ell^0_{tp}$.
...
the quality of the source blend entering a plant, the load remaining after the ordered parameter specific removal sequence $\mathcal{R}_{tp}$, and the final load after chemical dosing.
...
If $\mathcal{R}_{tp}=\varnothing$, then $n_{tp}=0$ and this load is simply the incoming load $\ell^0_{tp}$.
...
Only the processes in $\mathcal{R}_{t,\mathrm{turb}}$ are included in the ordered removal sequence for turbidity; installed processes that do not act on turbidity are excluded from this parameter specific sequence. The initial chemical dosing formulation does not assume that adding a coagulant produces a fixed linear reduction in turbidity; such a treatment response depends on water quality and operating conditions and therefore requires a separately justified or calibrated relationship \cite{Huang2025CoagulantDosing}. Accordingly, it is not represented by $\eta_{c,\mathrm{turb}}$ unless supporting data are available.
...
The units of $\eta_{cp}$ are defined so that $\eta_{cp}m_{tc}$ has the same load units as the existing quality load $\ell^{n_{tp}}_{tp}$. Thus,
...
\begin{itemize}
    ...
    \item Chemical dosing is applied after the ordered parameter-specific removal sequence $\mathcal{R}_{tp}$ in the initial formulation.
...
\begin{itemize}
    \item $\mathcal{R}_t$ contains all removal processes installed at plant $t$.
    \item For each parameter $p$, $\mathcal{R}_{tp}$ is the ordered subsequence of installed processes that act on that parameter.
    \item A process $r\in\mathcal{R}_t$ may appear in more than one $\mathcal{R}_{tp}$ when it acts on multiple quality parameters.
    \item Each relevant removal stage acts only on the load left by the preceding relevant stage for that parameter.
    \item The maximum removable fraction $H_{trp}$ is known for each supported plant-process-parameter combination.
    \item The total plant inflow receives the same installed treatment configuration.
    \item No water volume is lost as water passes through the removal processes.
    \item A process that does not act on parameter $p$ is excluded from $\mathcal{R}_{tp}$, equivalently having zero removal effect on $p$.
    \item Removal processes can only reduce quality load; required increases in supported parameters may instead be provided by the chemical dosing formulation.
\end{itemize}
...
\begin{itemize}
    ...
    \item Chemical dosing is represented after the parameter-specific removal sequence $\mathcal{R}_{tp}$.
...
The outgoing water volume is therefore $\sum_s a_s$, not a quality-load quantity such as $\ell^{n_{tp}}_{tp}$.
