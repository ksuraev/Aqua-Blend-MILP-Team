...
Source, plant, removal-process, and chemical-dosing activation decisions are binary, while water flows, removal loads, and chemical masses are continuous.
Transfer links are represented directly by continuous flow variables and capacity limits because the current formulation does not assign fixed activation costs to arcs.
...
\begin{tabularx}{\textwidth}{@{}lX@{}}
    ...
    $\mathcal{R}_t$    & all removalprocesses installed at plant $t$ regardless of what parameters they act on, $\mathcal R_t \subseteq \mathcal{R}$ \\
    $\mathcal{R}_{tp}$ & removal processes installed at plant $t$ acting on parameter $p$, as an ordered sequence
    $r_1, \dots, r_{n_{tp}}$ drawn from $\mathcal{R}_t$, in the order water
    flows through them                                                                                                                               \\
...
\end{tabularx}
$n_{tp} = |\mathcal{R}_{tp}|$ is written with the index because plants differ in how many processes they run, and how many processes in a plant act on a parameter.
For example, a plant with three processes removing alkalinity has $n_{t, \mathrm{alk}} = 3$, while one with a single removal process diminishing turbidity alone has $n_{t, \mathrm{turb}} = 1$.

The plant-specific set $\mathcal{K}_t$ represents chemical availability at plant $t$.
Unlike the parameter-specific ordered removal sequence $\mathcal{R}_{tp}$, $\mathcal{K}_t$ is an unordered subset containing only the chemicals and additives that can be dosed at plant $t$.
...
\begin{tabularx}{\textwidth}{@{}lXl@{}}
    ...
    $\eta_{cp}$          & effect on quality parameter $p$ per unit mass of chemical $c$                                           & $((\text{units of }p)\cdot\text{ML})/\text{kg}$ \\
\end{tabularx}
...
Direct pH adjustment is not represented by a constant linear chemical effect coefficient in this formulation.
...
\begin{tabularx}{\textwidth}{@{}llll@{}}
    ...
    $e_{trp}$         & $\ge 0$                     & load of parameter $p$ removed by process $r$ at plant $t$   & (units of $p$)$\cdot$ML/day \\[0.4em]
    \midrule
    \multicolumn{4}{@{}l}{\emph{Water quality loads}}                                                                                              \\
    $\ell^i_{tp}$     & $\ge 0$                     & quality load of parameter $p$ at treatment stage $i$ at plant $t$
...
\end{tabularx}
$\sigma_{tr}$ and $e_{trp}$ are defined only for $r \in \mathcal{R}_{tp}$.
The load variable $\ell^i_{tp}$ is defined for $i=0,\dots,n_{tp}+1$.
Stage $i=0$ represents the incoming quality load, stages $i=1,\dots,n_{tp}$ represent the loads remaining after the parameter-specific removal processes, and stage $i=n_{tp}+1$ represents the final treated load after supported chemical additions.
Since we have $\mathcal{R}_{tp} = (r_1, \dots, r_{n_{tp}})$ (Table~\ref{tab:sets}), the process at removal stage $i$ is $r_i$:
variables are indexed by the process itself, so $e_{t r_i p}$ is the same variable as $e_{trp}$ with $r = r_i$.

The chemical variables $\kappa_{tc}$ and $m_{tc}$ are defined only for $c \in \mathcal{K}_t$.
...
This keeps chemical cost and supported quality load additions linear.
...
\begin{center}
    ...
    \captionof{figure}{One example of the network structure (sources $\to$ treatment plants $\to$ demand zones).
        Each source-plant arc has a flow variable $b_{st}$; each plant-zone arc has $c_{tz}$.
...
\begin{equation}\label{eq:objective}
    \begin{aligned}
        \min \quad
                     & \sum_{s \in \mathcal{S}}
        F_s \alpha_s &                          & \text{source fixed cost} \\
        ...
        + \; & \sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st}\, b_{st}
              + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz}\, c_{tz}
              & & \text{arc transfer cost} \\
        + \; & \sum_{t \in \mathcal{T}} \sum_{r \in \mathcal{R}_t} F_{tr} \sigma_{tr}
              & & \text{removal process fixed cost} \\
        + \; & \sum_{t \in \mathcal{T}} \sum_{p \in \mathcal{P}} \sum_{r \in \mathcal{R}_{tp}}
              C_{trp}\, e_{trp}
              & & \text{removal cost} \\
        + \; & \sum_{t \in \mathcal{T}} \sum_{c \in \mathcal{K}_t}
              C^{\mathrm{chem}}_{tc}\, m_{tc}
              & & \text{chemical dosing cost}
    \end{aligned}
\end{equation}

\section{Constraints} \label{sec:constraints} \paragraph{Non-negativity.}
...
\begin{aligned}
    ...
    e_{trp} &\ge 0,
    && \forall t \in \mathcal{T},\ \forall p \in \mathcal{P},\ \forall r \in \mathcal{R}_{tp},\\
    \ell^i_{tp} &\ge 0,
    && \forall t \in \mathcal{T},\ \forall p \in \mathcal{P},\ i=0,\dots,n_{tp}+1,\\
    m_{tc} &\ge 0,
...
\begin{equation}\label{eq:plant_flow_conservation}
...
\begin{equation}\label{eq:source_flow_conservation}
    a_s = \sum_{t \in \mathcal{T}} b_{st} , \quad \forall s \in \mathcal{S}.
\end{equation}

\paragraph{Arc capacity.}
Each arc has a maximum flow and can carry water only if that source or plant is active.
For a source-to-plant arc, the flow $b_{st}$ is bounded by the arc capacity $\overline{L}_{st}$, the source's maximum withdrawal $\overline{W}_s$, and the plant's maximum throughput $\overline{V}_t$, and is zero unless the source is active ($\alpha_s = 1$).
\begin{equation}\label{eq:arc_capacity_st}
    b_{st} \le \min \left\{\overline{L}_{st}, \overline{W}_s, \overline{V}_t \right\} \alpha_s, \quad \forall s \in \mathcal{S}, t \in \mathcal{T}.
\end{equation}
Similarly, the flow $c_{tz}$ on a plant-to-zone arc is bounded by the arc capacity $\overline{L}_{tz}$ and the plant's maximum throughput $\overline{V}_t$, and is zero unless the plant is active ($\beta_t = 1$).
\begin{equation}\label{eq:arc_capacity_tz}
    c_{tz} \le \min \left\{\overline{L}_{tz}, \overline{V}_t\right\} \beta_t, \quad \forall t \in \mathcal{T}, z \in \mathcal{Z}.
\end{equation}
...
\begin{equation}\label{eq:removal_process_balance}
    ...
    \qquad \forall t \in \mathcal{T},\ \forall p \in \mathcal{P},\ i = 1, \dots, n_{tp}.
\end{equation}
The load remaining after the final relevant removal process is  $\ell^{n_{tp}}_{tp}$.
...
The constraint forces $e_{tr_ip}=0$ when $\sigma_{tr_i}=0$; when $\sigma_{tr_i}=1$, it reduces to a valid upper bound on the maximum removable load and therefore does not further restrict the feasible removal, formal derivations for $\sigma_{tr_i} = 1$ is given below.
...
One may notice that, $\sigma_{t r_i} = 1$, \eqref{eq:removal_activation} is immediately attained:
\begin{equation*}
    e_{t r_i p}
    ~ \underbrace{\le H_{t r_i p}\,\ell^{i-1}_{tp}}_{\eqref{eq:removal_capability}}
    ~ \underbrace{\le H_{t r_i p}\,\ell^{0}_{tp}}_{\eqref{eq:removal_process_balance}}
    ~ \underbrace{\le H_{t r_i p} \sum_{s \in \mathcal S}
        Q_{sp} b_{st}}_{\eqref{eq:removal_process_initial}} ~ \underbrace{\le H_{t r_i p}\,\overline{I}_{tp} \sum_{s \in \mathcal{S}} b_{st}}_{\eqref{eq:inlet_treatability}} ~ \underbrace{\le H_{t r_i p}\,\overline{I}_{tp}\,\overline{V}_t \beta_t}_{\eqref{eq:plant_capacity}}, \end{equation*} so \eqref{eq:removal_activation} restricts the model only through $\sigma_{t r_i} = 0$.


\paragraph{Integration with the chemical treatment stage.}
The removal-process equations above are retained unchanged.
When chemical dosing is enabled, $\ell^{n_{tp}}_{tp}$ is the load passed from the removal process sequence to the subsequent chemical treatment stage; the final plant outlet load is defined below as $\ell^{n_{tp}+1}_{tp}$.
...
The formulation assumes that chemical dosing occurs as a separate treatment stage following the ordered parameter specific removal sequence $\mathcal{R}_{tp}$.
...
The final treatment stage load extends the existing load sequence by one stage:
\begin{equation}\label{eq:final_treatment_load}
    \ell^{n_{tp}+1}_{tp}
    =
    \ell^{n_{tp}}_{tp}
    +
...
\end{equation}
If $\mathcal{R}_{tp}=\varnothing$, then $n_{tp}=0$ and the final stage becomes
\[
\ell^1_{tp}
=
\ell^0_{tp}
...
The water leaving each plant must remain within the regulatory limits after both removal processes and supported chemical additions have been applied.
For parameter $p$, $\ell^{n_{tp}+1}_{tp}$ is the final treated quality load, while $\sum_s b_{st}$ is the corresponding water volume.
...
the quality of the source blend entering a plant, the load remaining after the ordered parameter specific removal sequence $\mathcal{R}_{tp}$, and the final treated load $\ell^{n_{tp}+1}_{tp}$ after supported chemical additions.
...
The incoming load then passes through the ordered parameter specific removal sequence $\mathcal{R}_{tp}$ according to
\eqref{eq:removal_process_initial}--\eqref{eq:removal_process_balance}.
After the final removal stage relevant to parameter $p$, the remaining load is $\ell^{n_{tp}}_{tp}$.
If $\mathcal{R}_{tp}=\varnothing$, then $n_{tp}=0$ and this removal-stage load is simply the incoming load $\ell^0_{tp}$.
Supported chemical contributions are then incorporated through~\eqref{eq:final_treatment_load}, producing the final treated load $\ell^{n_{tp}+1}_{tp}$.
...
In water supply modelling, turbidity is important because high turbidity can increase treatment requirements and affect disinfection performance \cite{WHOTurbidity2011, Matos2024Turbidity, UMNTurbidity}.
...
Only the processes in $\mathcal{R}_{t,\mathrm{turb}}$ are included in the ordered removal sequence for turbidity; installed processes that do not act on turbidity are excluded from this parameter specific sequence. This formulation does not assume that adding a coagulant produces a fixed linear reduction in turbidity; such a treatment response depends on water quality and operating conditions and therefore requires a separately justified or calibrated relationship \cite{Huang2025CoagulantDosing}. Accordingly, it is not represented by $\eta_{c,\mathrm{turb}}$ unless supporting data are available.
...
The final treated alkalinity load at plant $t$ is
\begin{equation}\label{eq:alkalinity_post_treatment}
    \ell^{n_{t,\mathrm{alk}}+1}_{t,\mathrm{alk}}
    =
    \ell^{n_{t,\mathrm{alk}}}_{t,\mathrm{alk}}
    +
...
In this formulation, removal processes do not act directly on pH.
For every installed process,
\[
H_{tr,\mathrm{pH}}=0
\qquad
\forall t\in\mathcal{T},\ r\in\mathcal{R}_t,
\]
so the parameter specific removal sequence is empty:
\[
\mathcal{R}_{t,\mathrm{pH}}=\varnothing,
\qquad
n_{t,\mathrm{pH}}=0.
\]
Consequently,
\[
\ell^{n_{t,\mathrm{pH}}}_{t,\mathrm{pH}}
=
\ell^0_{t,\mathrm{pH}},
\]
and the transformed pH load passes unchanged through the removal part of the formulation.
...
\]
in this formulation.
Since $n_{t,\mathrm{pH}}=0$ and the chemical contribution is zero, the final treatment-stage load is
\[
\ell^{n_{t,\mathrm{pH}}+1}_{t,\mathrm{pH}}
=
\ell^1_{t,\mathrm{pH}}
=
\ell^0_{t,\mathrm{pH}}.
...
In this formulation, pH is not modified by either removal processes or chemical dosing, so the recovered blended pH is also the modelled plant outlet pH.
...
The units of $\eta_{cp}$ are defined so that $\eta_{cp}m_{tc}$ has the same load units as the pre-chemical removal-stage load $\ell^{n_{tp}}_{tp}$ and the final treated load $\ell^{n_{tp}+1}_{tp}$. Thus,
...
This includes relationships, such as direct chemical adjustment of pH, that are not represented by the linear chemical dosing formulation.
...
\begin{enumerate}[label=(\roman*), align=left, widest=vii]
    ...
    \item Raw $\mathrm{pH}$ values are not averaged directly; $\mathrm{pH}$ is transformed to scaled hydrogen ion concentration in nmol/L before optimisation.
    \item Nonlinear interactions between blended source-water quality parameters are not modelled.
    \item Water quality is checked at each treatment plant: incoming quality is validated against $\underline{I}_{tp}$ and $\overline{I}_{tp}$, while final treated quality is validated against $\underline{Q}_p$ and $\overline{Q}_p$ using constraint~\eqref{eq:outlet_water_quality}.
\end{enumerate}

These assumptions allow the general water-quality constraints to remain linear and suitable for inclusion in the MILP formulation.

\subsection{Assumptions for removal processes}
\label{sec:removal_process_axiom}
...
\begin{enumerate}[label=(\roman*), align=left, widest=viii]
    \item Removal processes $\mathcal R_{tp}$ are conducted in a sequence.
    \item A removal process $r_i \in \mathcal R_{t}$ can remove more than one $p \in \mathcal P$, appearing as $r_i \in \mathcal R_{tp}$ for each $p$.
    \item Every process $r_i$ is only dependent on its preceeding process $r_{i - 1}$, see \eqref{eq:removal_process_balance}.
    \item Maximum removal efficiency $H_{trp}$ is known.
    \item Uniform treatment: $\sum_{s \in \mathcal S} b_{st}$ receives the same treatment.
    \item No water loss after passing through $r_i \in \mathcal R_{tp}$.
    \item $\mathcal{R}_{tp}$ is an ordered sequence induced by $\mathcal{R}_{t}$.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*), align=left, widest=vii]
    ...
    \item The formulation assumes that chemical dosing occurs as a separate stage following the parameter-specific removal sequence $\mathcal{R}_{tp}$.
    ...
    \item Chemical interactions, reaction kinetics, decay, and other nonlinear dose response behaviour are outside the MILP unless represented by a separately justified linear approximation.
\end{enumerate}

The assumptions in Section~\ref{sec:removal_process_axiom} apply specifically to the removal process subsystem.
The chemical dosing stage is modelled separately after that sequence, so supported chemical additions may adjust lower-bound quality requirements without changing the removal process equations.
...
With one plant and one zone there is no routing choice:
the flow variables are fixed by conservation, while the source decisions and any applicable removal and chemical dosing decisions remain free.
...
\begin{center}
    \begin{tabular}{@{}ll@{}}
        ...
        $\mathcal{R}_{tp}$ and $\mathcal{K}_t$ fixed & the plant inherits its removal and chemical dosing configuration \\
        \bottomrule
    \end{tabular}
\end{center}

The routing variables are therefore determined by the source withdrawals, but treatment decisions are still inherited from the general model.
Where applicable, these include process activation $\sigma_{tr}$, removal loads $e_{trp}$, chemical activation $\kappa_{tc}$, and chemical mass $m_{tc}$.
...
\begin{center}
    ...
    \captionof{figure}{Toy model.
        Three sources draw $a_1,a_2,a_3$ into the single plant $t$, which delivers their total water volume to the single zone $z$.}
\end{center}

Removal and chemical dosing change quality loads but, under the current assumptions, do not change the water volume flow balance.
The outgoing water volume is therefore $\sum_s a_s$, not a quality load quantity such as the final treated load $\ell^{n_{tp}+1}_{tp}$.

\noindent Since this is just the network model with routing fixed, scaling up requires only freeing the routing variables and adding more sources, plants and zones.
The same treatment and chemical dosing formulation is inherited without reformulation.
...
\paragraph{Final quality bounds implied by removal and chemical treatment.}
Repeated application of the removal process balance \eqref{eq:removal_process_balance} across the ordered sequence $\mathcal{R}_{tp}=(r_1,\dots,r_{n_{tp}})$ gives
\begin{equation}\label{eq:implicit_removal_load}
    \ell^{n_{tp}}_{tp}
    =
    \ell^{0}_{tp}
    -
    \sum_{i=1}^{n_{tp}} e_{t r_i p},
    \qquad
    \forall t \in \mathcal{T},\ \forall p \in \mathcal{P}.
\end{equation}
Using the incoming load definition \eqref{eq:removal_process_initial}, this becomes
\begin{equation}\label{eq:implicit_removal_load_sources}
    \ell^{n_{tp}}_{tp}
    =
    \sum_{s \in \mathcal{S}} Q_{sp}\,b_{st}
    -
...
The chemical quality stage \eqref{eq:final_treatment_load} then adds the supported chemical contributions to the remaining removal-stage load.
Combining \eqref{eq:implicit_removal_load_sources}, \eqref{eq:final_treatment_load}, and the outlet quality constraint
\eqref{eq:outlet_water_quality} gives, for every $t \in \mathcal{T}$ and $p \in \mathcal{P}$,
\begin{equation}\label{eq:implicit_final_quality_bounds}
    \underline{Q}_p \sum_{s \in \mathcal{S}} b_{st}
    \le
    \ell^{n_{tp}+1}_{tp}
    =
...
Unlike the removal only case, a positive lower regulatory limit does not necessarily have to be met by source selection and blending alone.
Removal processes can only reduce the load passed through the removal sequence, while supported chemical additions with $\eta_{cp}>0$ can increase the final treated load.
Three cases follow.
\begin{itemize}
    \item If $\underline{Q}_p = 0$, the lower bound in \eqref{eq:implicit_final_quality_bounds} is implied by the non-negativity of $\ell^{n_{tp}+1}_{tp}$ in \eqref{eq:nonnegativity}.

    \item If $\eta_{cp}>0$ for at least one $c\in\mathcal{K}_t$, chemical dosing may contribute to satisfying the lower regulatory limit through the final treatment-stage relation \eqref{eq:final_treatment_load}.

    \item If no supported chemical effect exists for parameter $p$ at plant $t$, so that
          \begin{equation*}
              \eta_{cp}=0,
              \qquad
              \forall c\in\mathcal{K}_t,
          \end{equation*}
          then \eqref{eq:final_treatment_load} gives
          \begin{equation*}
              \ell^{n_{tp}+1}_{tp}
              =
              \ell^{n_{tp}}_{tp}.
          \end{equation*}
          Since removal processes only reduce load,
          \begin{equation*}
              \ell^{n_{tp}}_{tp}
              \le
              \ell^0_{tp}
              ...
              \sum_{s\in\mathcal{S}}Q_{sp}b_{st},
          \end{equation*}
          so a positive lower regulatory limit must already be satisfied by the incoming blend.
          If, in addition, no removal process acts on $p$ (i.e.\ $\mathcal{R}_{tp}=\emptyset$ and $n_{tp}=0$), then
          \begin{equation*}
              \ell^{n_{tp}+1}_{tp}
              =
              \ell^0_{tp}
              ...
              \sum_{s\in\mathcal{S}}Q_{sp}b_{st},
          \end{equation*}
          and the outlet quality constraint reduces to
          \begin{equation*}
              \underline{Q}_p \sum_{s \in \mathcal{S}} b_{st}
              \le
              \sum_{s \in \mathcal{S}} Q_{sp}\,b_{st}
              ...
              \overline{Q}_p \sum_{s \in \mathcal{S}} b_{st},
          \end{equation*}
          so both regulatory limits must be met by blending alone.
\end{itemize}

\paragraph{Inactive plants cannot dose chemicals.}
Combining the chemical mass activation constraint \eqref{eq:chemical_mass_activation} with the chemical activation constraint \eqref{eq:chemical_activation} gives
\begin{equation}\label{eq:implicit_chemical_plant_activation}
    m_{tc}
    ~\underbrace{\le
    \overline{\rho}_{tc}\,\overline{V}_t\,\kappa_{tc}}_{\eqref{eq:chemical_mass_activation}}
    ~\underbrace{\le
    \overline{\rho}_{tc}\,\overline{V}_t\,\beta_t}_{\eqref{eq:chemical_activation}},
    \qquad
    \forall t \in \mathcal{T},\ \forall c \in \mathcal{K}_t.
\end{equation}
Therefore, if plant $t$ is inactive ($\beta_t=0$), then \eqref{eq:chemical_activation} forces $\kappa_{tc}=0$ and \eqref{eq:chemical_mass_activation} forces $m_{tc}=0$.
Hence an inactive treatment plant cannot dose chemicals.
