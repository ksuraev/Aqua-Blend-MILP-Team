...
All costs, capacities, and demands are assumed non-negative.
The units of a water quality parameter depend on $p$.
Parameters that are not linear in their natural units are stored in $Q_{sp}$ as a transformed value that blends approximately linearly.
...
the fixed cost of activating each source and plant, and the
per-ML costs of drawing water, treating it (charged on the volume each plant processes, $\sum_s b_{st}$) and transferring it along each link.

\begin{equation}
	\min
	\underbrace{\sum_{s \in \mathcal{S}}
		F_s \alpha_s}_{\text{source activation}} + \underbrace{\sum_{t \in \mathcal{T}} F_t \beta_t}_{\text{plant activation}} + \underbrace{\sum_{s \in \mathcal{S}} C_s a_s}_{\text{drawing water}} + \underbrace{\sum_{t \in \mathcal{T}} C_t \sum_{s \in \mathcal{S}} b_{st}}_{\text{plant treatment}} + \underbrace{\sum_{s \in \mathcal{S}} \sum_{t \in \mathcal{T}} C_{st} b_{st} + \sum_{t \in \mathcal{T}} \sum_{z \in \mathcal{Z}} C_{tz} c_{tz}}_{\text{transfer}}.
\end{equation}
...
In water supply modelling, turbidity is important because high turbidity may increase treatment requirements and affect disinfection performance.
...
Alkalinity measures the ability of water to neutralise acid.
It represents the buffering capacity of water and is relevant to $\mathrm{pH}$ stability, corrosion control, treatment performance, remineralisation, and water stability in distribution systems.
...
To keep the MILP linear, $\mathrm{pH}$ is handled as a preprocessing transformation.
...
The direction of the bounds changes because $\mathrm{pH}$ and hydrogen ion concentration are inversely related.
