...
\textcolor{magenta}{also each process sees only what the previous left?}
\textcolor{red}{Added, and refers to \eqref{eq:removal_process_balance}}
...
\begin{itemize}
    ...
    \item Maximum removal efficiency $H_{trp}$ is known. \textcolor{red}{I'm thinking about a weaker assumption, not maximum but a fixed efficiency is known?} \textcolor{magenta}{keep maximum I think $\epsilon \le H \ell$ is an upper bound, so the process can remove less. Fixed would be $\epsilon = H \ell$, which forces removal whenever the process runs and ruins the behaviour where the model stops at the limit rather than at capacity.} \textcolor{blue}{Yup, makes sense :>>}
