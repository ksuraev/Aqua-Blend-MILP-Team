...
We highlight here that, according to \cite{who2004water}, removal process can only reduce alkalinity, though it can be increased by an adjustment process.\textcolor{red}{which is not currently modelled}
...
\textcolor{red}{With the new indexing, $\mathcal{R}_{t,\mathrm{pH}}=\emptyset$, and $n_{t,\mathrm{pH}}=0$ so the removal constraints generate no instances and $\ell^{n_{tp}}=\ell^{0}$ follows directly?}
...
\begin{enumerate}[label=(\roman*), align=left, widest=viii]
    ...
    \item Water quality is checked at each treatment plant, using the blend of its incoming sources. \textcolor{red}{now checked at inlet against $I$ and at outlet against $Q$}
\end{enumerate}
...
\begin{enumerate}[label=(\roman*), align=left, widest=viii]
    ...
    \item A removal process $r_i \in \mathcal R_{tp}$ can remove more than one $p \in \mathcal P$. \textcolor{red}{maybe: a process may act on several parameters, appearing in $\mathcal R_{tp}$ for each $p$ it acts on}
    ...
    \item \textcolor{red}{treatment only reduces a load, lower limits met by blending alone}
    \item \textcolor{red}{$\mathcal{R}_{tp}$ preserves the order of $\mathcal{R}_{t}$?}
\end{enumerate}
