...
\textcolor{red}{maybe worth saying the lower limit for alk can't be met by treatment at all i.e. if no blend reaches it, the model is infeasible.}
\textcolor{blue}{Hmmm, I agree to flag the model's infeasibility when no blend reaches the lower limits, if so, then we had better fix the generic formulation \eqref{eq:inlet_treatability} instead, 'cuz I think it is also true for other $p \in \mathcal P$?
    Hmmm, but later on, we will add other treatment processes, so perhaps, I will highlihgt if the model only installs removal process?
}
\textcolor{red}{I'd maybe just write ``We highlight here that, according to \cite{who2004water}, removal processes can only reduce alkalinity.'' though if adding chemical dosing then this is no longer true. Can add ``Therefore, with only removal process installed, infeasibility is an immediate result since removal processes cannot increase the $p \in \mathcal P$.'' somewhere earlier if we don't already though. Hopefully understood what you mean here otherwise msg me :)}
...
\textcolor{blue}{This is what I intend to include in the docs - }
Therefore, with only removal process installed, infeasibility is an immediate result since removal processes cannot increase the $p \in \mathcal P$.


\subsection{pH transformation}

\textcolor{red}{Turbidity and Alkalinity should be lowercase}
...
\textcolor{red}{I wouldn't maybe say that they are inapplicable but rather that the removal constraints still apply but have no effect?}
\textcolor{blue}{Ah yeah, I agree, I will state that the constraints still hold if $e_{tr_i p} = 0$ w whenever $p = pH$}
\textcolor{red}{should be by setting $H_{tr,pH}=0$ which forces $e_{tr,pH}=0$ in the constraints, rather than setting $e_{tr,pH}=0$ directly.}

As mentioned earlier, pH cannot be handled by removal process; however, constraints from \eqref{eq:removal_process_initial} to \eqref{eq:outlet_water_quality} still hold, by setting $e_{tr,\mathrm{pH}} = 0, \forall r \in \mathcal R_t$.
