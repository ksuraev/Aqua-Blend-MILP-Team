...
def test_constraint_indices_match_network(
    ...
    assert set(model.plant_zone_link_capacity) == {("T1", "Z1")}
    ...
    assert set(model.plant_link_activation) == {("T1", "Z1")}

    assert set(model.quality_lower_bound) == {("T1", "turbidity")}

    assert set(model.quality_upper_bound) == {("T1", "turbidity")}
...
def test_unused_plant_is_not_forced_to_its_minimum(
    two_plant_parameters: ModelParameters,
) -> None:
    """Check that an inactive plant is not forced to meet its minimum throughput."""
    parameters = replace(
        two_plant_parameters,
        plant_min_throughput={"T1": 0.0, "T2": 12.0},
        plant_fixed_cost={"T1": 5.0, "T2": 500.0},
    )
...
def test_lower_quality_limits_apply_per_plant_not_network_wide(
    separate_plants_parameters: ModelParameters,
) -> None:
